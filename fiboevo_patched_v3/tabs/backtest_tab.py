...
def render():
    ...
    if st.button("Run quick backtest (last segment)"):
        ...
        # Ensure model and meta are present
        if not model_path:
            st.error("No model selected in session. Ve a la pestaña Entrenamiento y carga/entrena un modelo.")
            return
        if not meta:
            st.error("No meta information available for the selected model.")
            return
