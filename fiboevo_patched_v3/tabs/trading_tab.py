...
def prepare_model_input(df_segment: pd.DataFrame, feature_cols: list, seq_len: int, scaler_path: str = None):
    ...
    if df_segment is None or len(df_segment) < seq_len:
        raise ValueError(f"Insufficient history: need >= seq_len rows, got {len(df_segment) if df_segment is not None else 0}")

    # select last seq_len rows as DataFrame to preserve column names
    seg = df_segment.iloc[-seq_len:].copy().reset_index(drop=True)
    X_df = seg[feature_cols].copy()

    # apply scaler if provided
    if scaler_path and Path(scaler_path).exists() and joblib is not None:
        try:
            scaler = joblib.load(scaler_path)
            # preferir pasar DataFrame con columnas para evitar warnings y validar orden
            expected = None
            if hasattr(scaler, "feature_names_in_"):
                expected = list(scaler.feature_names_in_)
            if expected is None and isinstance(feature_cols, list):
                expected = list(feature_cols)
            if expected is not None:
                if set(expected) != set(X_df.columns):
                    missing = [c for c in expected if c not in X_df.columns]
                    if missing:
            ...
            # fallback: use raw values but warn
            st.warning(f"Fallo al cargar/transformar con scaler: {e} — se usará X sin escalar.")
            X = X_df.values.astype(np.float32)
    else:
        X = X_df.values.astype(np.float32)

    # final shape (1, seq_len, features)
    return np.expand_dims(X, axis=0)


def predict_with_model(model_path: str, X_numpy, meta_path: str = None, scaler_path: str = None, allow_partial_load: bool = False):
    """
    Carga modelo de fibo.load_model respetando meta si existe. Devuelve dict con predicciones.
    X_numpy expected shape: (1, seq_len, features)
    """
    import torch
    ...
    # try to find meta: look for model_path.with_suffix('.json') or provided meta_path
    meta = None
    cand_meta = Path(meta_path) if meta_path else p.with_suffix('.json')
    if cand_meta.exists():
        try:
            with open(cand_meta, 'r', encoding='utf-8') as f:
                meta = json.load(f)
        except Exception as e:
            st.warning(f"Could not read meta json {cand_meta}: {e}")

    # prefer session_state meta if present
    if st.session_state.get('meta'):
        meta = st.session_state['meta']

    input_size = X_numpy.shape[2]
    hidden = None
    if meta and 'hidden' in meta:
        try:
    ...
    try:
        model = fibo.load_model(p, input_size=input_size, hidden=hidden, device=device, allow_partial_load=allow_partial_load)
    except Exception as e:
        # fallback: try legacy loading (raw state_dict) into default LSTM2Head
        try:
            state = torch.load(p, map_location=device)
            if isinstance(state, dict) and 'state_dict' in state:
                state = state['state_dict']
            model = fibo.LSTM2Head(input_size=input_size, hidden_size=hidden or 64)
            model.load_state_dict(state)
            model.to(device)
        except Exception as e2:
            raise RuntimeError(f"No se pudo cargar el modelo con fibo.load_model ni fallback: {e2}") from e

    model.eval()
    with torch.no_grad():
        xt = torch.from_numpy(X_numpy.astype(np.float32)).to(device)
        out = model(xt)
        # model may return tuple (ret, vol) or single tensor
        if isinstance(out, tuple) or isinstance(out, list):
            out_ret = out[0]
            out_vol = out[1] if len(out) > 1 else None
        else:
            out_ret = out
            out_vol = None
        ret = float(out_ret.cpu().numpy().ravel()[0])
        vol = float(out_vol.cpu().numpy().ravel()[0]) if out_vol is not None else None
    return {'pred_ret': ret, 'pred_vol': vol}
