...
def load_data_from_influx(
    ...
    return_df: bool = False) -> np.ndarray:
    if not _HAS_INFLUX:
    ...
    with InfluxDBClient(url=INFLUX_URL, token=INFLUX_TOKEN, org=INFLUX_ORG) as client:
        ...
        for table in tables:
            for record in table.records:
                # try to obtain both value and timestamp from the record
                try:
                ...
                try:
                    ...
                    try:
                        t = getattr(record, 'get_time', None)
                        if callable(t):
                            t = t()
                    except Exception:
                        t = None
    ...
    if return_df:
        ...
        # attempt to parse times to datetime; if times are None, create a range index
        try:
            times_parsed = _pd.to_datetime(times)
        except Exception:
            times_parsed = None
        if times_parsed is None or any(t is None for t in times):
            _df = _pd.DataFrame({field: values})
        else:
            _df = _pd.DataFrame({field: values}, index=times_parsed)
            _df.index.name = 'time'
        return _df
...
def save_model(model: nn.Module, path_model: Path, meta: Optional[dict] = None, path_meta: Optional[Path] = None) -> None:
    """
    Guarda un dict con state_dict y meta (si se provee), para que luego la carga sea determinista.
    """
    path_model = Path(path_model)
    payload = {"state_dict": model.state_dict()}
    if meta is not None:
        payload["meta"] = meta
    torch.save(payload, path_model)
    # guardamos meta también en JSON separado si path_meta se ofrece (compatibilidad)
...
def _infer_config_from_checkpoint(checkpoint: dict) -> dict:
    """
    Inferir parámetros principales del state_dict si meta no está presente.
    Trata de deducir input_size y hidden del tamaño de weight_ih_l0 si es un LSTM.
    """
    ...
    config = {}
    # buscar la primera key que contenga 'lstm.weight_ih_l0'
    for k, v in sd.items():
...
def _safe_partial_load_state_dict(model: nn.Module, checkpoint_state: dict):
    """
    Copia las porciones compatibles entre checkpoint_state y model.state_dict().
    Para tensores 2D copia el sub-bloque superior-left que encaje; para vectores copia la porción inicial.
    IMPORTANTE: esto es una medida de emergencia: puede funcionar para transfer learning ligero, pero
    es preferible entrenar o volver a guardar checkpoints con la arquitectura correcta.
    """
    ms = model.state_dict()
    new_state = {}
    for k, v in checkpoint_state.items():
        ...
        try:
            if v.ndim == 2:
                ...
                LOGGER.warning("Cannot handle tensor with ndim=%d for key %s; leaving target init.", v.ndim, k)
...
def load_model(path_model: Path, input_size: Optional[int] = None, hidden: Optional[int] = None, device: torch.device = torch.device("cpu"), allow_partial_load: bool = False) -> LSTM2Head:
    """
    Carga un modelo desde path_model. Intenta reconstruir la arquitectura usando meta embebido en el checkpoint,
    o deduciendo dimensiones desde el state_dict. Si se detecta mismatch de shapes, usa safe partial load si se permite.
    """
    path_model = Path(path_model)
    checkpoint = torch.load(path_model, map_location=device)
    # checkpoint puede ser {'state_dict':..., 'meta': {...}} o directamente state_dict
    if isinstance(checkpoint, dict) and "state_dict" in checkpoint:
        state_dict = checkpoint["state_dict"]
        ch_meta = checkpoint.get("meta", None)
    else:
        state_dict = checkpoint
        ch_meta = None

    inferred = _infer_config_from_checkpoint(checkpoint if isinstance(checkpoint, dict) else {"state_dict": state_dict})
    final_input_size = input_size if input_size is not None else inferred.get("input_size")
    final_hidden = hidden if hidden is not None else (inferred.get("hidden") or 64)

    if final_input_size is None:
        raise RuntimeError("No se puede determinar input_size para instanciar el modelo. Provee input_size al llamar load_model o guarda meta en el checkpoint.")

    model = LSTM2Head(input_size=final_input_size, hidden_size=final_hidden).to(device)

    try:
        model.load_state_dict(state_dict)
        LOGGER.info("Loaded state_dict strictly for %s", path_model)
    except RuntimeError as e:
        LOGGER.warning("Strict load failed: %s", e)
        if allow_partial_load:
            LOGGER.info("Attempting safe partial load.")
